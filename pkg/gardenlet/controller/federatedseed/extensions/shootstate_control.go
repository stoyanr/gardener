	...
	return func(req reconcile.Request) (reconcile.Result, error) {
		...
		if _, err := controllerutil.CreateOrUpdate(ctx, s.k8sGardenClient.Client(), shootState, func() error {
			...
			for _, currentResource := range currentResources {
				if getResourceByName(resources, currentResource.Name) == nil {
...
func getShootStateExtensionState(shootState *gardencorev1alpha1.ShootState, kind string, name, purpose *string) (*runtime.RawExtension, []gardencorev1alpha1.NamedResourceReference) {
...
func updateShootStateExtensionState(shootState *gardencorev1alpha1.ShootState, kind string, name, purpose *string, state *runtime.RawExtension, resources []gardencorev1alpha1.NamedResourceReference) {
	list := gardencorev1alpha1helper.ExtensionResourceStateList(shootState.Spec.Extensions)
...
func getResourceByName(resources []gardencorev1alpha1.NamedResourceReference, name string) *gardencorev1alpha1.NamedResourceReference {
	for _, resource := range resources {
